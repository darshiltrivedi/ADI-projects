...
class App:  
    def __init__(self, master):      
        ...
        self.LO_freq   = config.Rx_freq           # RX LO freq of Pluto/ADRV9361
        ...
        self.sdr = SDR_init(self.sdr_address, self.num_Rx, self.SampleRate, self.TX_freq, self.LO_freq, self.Rx_gain, -3)
        print(self.sdr)
        ...
        slide_refresh_time=Scale(frame1, from_=300, to=5000, variable=self.update_time, resolution=100, troughcolor="LightYellow3", bd=2, orient=HORIZONTAL, relief=SUNKEN, length=200)
        ...
        check_PlotMax = tk.Checkbutton(frame6, text="Show Peak Gain", highlightthickness=0, variable=self.PlotMax_set, command=clearMax, onvalue=1, offvalue=0, anchor=W, relief=SUNKEN)
    ...
    def programBeam(self):
        ...
        self.MixerLO_text.set(str("Mixer LO Freq = "+str(int(self.TX_freq/1000000))+" MHz"))
        ...
        for PhDelta in PhaseValues:
            ...
            for count in range (0, self.Averages):
                ...
                '''with warnings.catch_warnings():
                    ...
                    data = signal.decimate(data_raw, 8, ftype='fir')
                    data = signal.decimate(data_raw, 8, ftype='fir')'''
    ...
    def plotData(self):
        ...
        self.ax1.plot(self.saved_angle, self.saved_gain, '-o', ms=1, alpha=0.5, mfc='red')
        ...
        self.pol_ax1.plot(np.radians(self.saved_angle), self.saved_gain, '-o', ms=1, alpha=0.5, mfc='red')
        ...
        '''if self.first == 0:
            self.lmax = np.full_like(self.max_gain, -1000)
            self.lmin = np.full_like(self.max_gain, 1000)
            self.first = 1
        self.lmax = np.maximum(self.lmax, self.max_gain)
        self.lmin = np.minimum(self.lmin, self.max_gain)
        self.ax3.plot(self.xf/1e6, self.lmax, ms=1, alpha=0.5, mfc='green')'''
